...
export function summarizeVoiceCloud(points) {
  ...
  return {
    ...
    medianHz: mid(Float64Array.from(pts, (p) => p.hz).sort()), // ⚡ Bolt: optimized numerical array sorting
    medianRes: mid(Float64Array.from(pts, (p) => clamp01(p.res)).sort()), // ⚡ Bolt: optimized numerical array sorting
  };
}
...
export function nucleusFromRun(run, { minFrames = 3 } = {}) {
  ...
  const vs = Float64Array.from(run, (s) => s.value).sort(); // ⚡ Bolt: optimized numerical array sorting
