...
export function computeFormantDispersion(formants) {
  ...
  // Performance optimization: Avoids O(N) .filter() array allocation.
  // The sum of consecutive differences (f1-f0) + (f2-f1) ... is a telescoping series,
  // algebraically simplifying to (last - first). We compute it in one pass.
  let first = -1;
  let last = -1;
  let count = 0;
  for (let i = 0; i < formants.length; i++) {
    if (formants[i] > 0) {
      if (first === -1) first = formants[i];
      last = formants[i];
      count++;
    }
  }
