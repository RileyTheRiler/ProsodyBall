...
export function computeFormantDispersion(formants) {
  ...
  // Optimization: The sum of consecutive differences (f[1]-f[0]) + (f[2]-f[1]) + ...
  // is a telescoping series that simplifies algebraically to (last - first).
  // This transforms O(N) allocation and looping into a single O(N) zero-allocation pass.
  let first = 0;
  let last = 0;
  let count = 0;
