...
export function computeFormantDispersion(formants) {
  ...
  // ⚡ Bolt: Telescoping series optimization.
  // Summing consecutive differences (f[1]-f[0] + f[2]-f[1] ...) algebraically simplifies
  // to (last - first). This single-pass loop avoids allocating a filtered array.
  let first = 0;
  let last = 0;
  let count = 0;
