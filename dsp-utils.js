...
export function computeFormantDispersion(formants) {
  ...
  // ⚡ Bolt: Replaced .filter().reduce() with a single-pass O(N) loop to eliminate array allocation
  // in the hot analyzer loop (reducing GC pauses).
  // 💡 Telescoping sum simplification: The mean adjacent spacing (f[1]-f[0] + f[2]-f[1]...)
  // mathematically simplifies to (last - first) / (count - 1).
  let first = -1;
  let last = -1;
  let count = 0;
