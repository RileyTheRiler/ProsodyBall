...
export function computeFormantDispersion(formants) {
  ...
  // ⚡ Bolt: Replace .filter() + loop with zero-allocation single pass.
  // Algebraically, the sum of differences (f[1]-f[0]) + (f[2]-f[1]) + ...
  // simplifies to just (last - first) as a telescoping series.
  let first = -1;
  let last = -1;
  let count = 0;
