...
## 2024-06-05 - [DOM-based XSS Risk via href property assignments]
**Vulnerability:** Found direct assignments of potentially tainted URLs (e.g. `window.location.href`) to `.href` properties of newly created anchor tags without sanitization. While safer than `innerHTML`, it still poses a risk for DOM-based XSS through dangerous protocols (`javascript:`, `data:`, etc.).
**Learning:** Direct property assignments of untrusted URLs are not fully safe against dangerous protocol injections. Although they prevent HTML breakout (unlike `innerHTML`), they can still execute scripts when the link is clicked if the protocol is malicious.
**Prevention:** Wrap all assignments to URL attributes (like `.href` or `.src`) with the `sanitizeUrl()` utility function when dealing with tainted sources like `window.location.href` or user inputs.
