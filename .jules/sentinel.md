...
## 2026-04-07 - [DoS Risk via Unbounded User Input]
**Vulnerability:** Found multiple text input vectors (`<textarea>` for custom passages and `window.prompt` for teleprompter) that allowed users to paste an unlimited amount of text.
**Learning:** Accepting unbounded user text in client-side applications can lead to browser hangs, out-of-memory errors, and UI freezing (a form of client-side Denial of Service), especially when that text is subsequently processed by complex rendering or analysis logic.
**Prevention:** Always enforce strict length limits on any free-form user input at the point of entry. Use the `maxlength` attribute on HTML text inputs and apply `.substring()` constraints on programmatic inputs like `window.prompt()` to bound resource consumption.
