...
## 2026-05-01 - [DOM-based XSS Risk via href Assignment]
**Vulnerability:** Untrusted inputs (like `window.location.href` or derived URLs) were directly assigned to `link.href` in `app.js`. If these inputs contained dangerous protocols (`javascript:`, `data:`, `vbscript:`), they could execute malicious code when the user clicked the link, bypassing browser URL-encoding protections.
**Learning:** Direct assignment to `.href` safely escapes quotes and encodes many characters, but it *does not* prevent execution of JavaScript protocols if the attacker fully controls the URL scheme. It is crucial to distinguish between safe (e.g. `http`, `https`) and dangerous protocols before assignment.
**Prevention:** Apply a `sanitizeUrl` utility that explicitly strips or replaces dangerous protocols (like `/^(%20|\s)*(javascript|data|vbscript):/i`) for all dynamic `href` assignments where the source is untrusted.
