...
## 2024-05-24 - [DOM-based XSS Risk via unsafe href assignment]
**Vulnerability:** Found `window.location.href` and `directUrl` being assigned directly to `link.href` without sanitization. While assigning to `.href` handles browser encoding and escapes quotes to prevent HTML attribute breakout, it remains vulnerable to DOM-based XSS if the URL contains dangerous protocols like `javascript:`, `data:`, or `vbscript:`.
**Learning:** `window.location.href` is an untrusted source. Even when using safe DOM assignments like `element.href = url`, malicious protocols can execute arbitrary JavaScript when the link is clicked. This is a common misunderstanding where developers assume direct property assignment is fully safe against all XSS vectors.
**Prevention:** Always implement a URL sanitization function (e.g., `sanitizeUrl`) that strips or invalidates dangerous protocols (like `javascript:`, `data:`, `vbscript:`) before assigning any untrusted URL to an `href` attribute. Apply this even when the source seems benign (like the current window location).
