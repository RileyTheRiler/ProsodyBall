...
## 2026-05-24 - DOM-based XSS via window.location.href assignment
**Vulnerability:** Found multiple DOM-based XSS vulnerabilities in `app.js` where `window.location.href` and `directUrl` were passed unsanitized into `link.href` properties inside dynamically created DOM elements (`iframeNotice` and `showError`).
**Learning:** Directly assigning tainted URLs to the `.href` property of DOM elements is still vulnerable to DOM-based XSS via dangerous protocol injection (`javascript:`, `data:`, `vbscript:`), even if `innerHTML` string concatenation is avoided.
**Prevention:** Implement a central URL sanitization utility (e.g., `sanitizeUrl` in `dsp-utils.js`) and wrap all dynamic assignments to `.href` properties with it to ensure dangerous protocols are neutralized (e.g., replaced with `about:blank`).
