...
## 2024-05-30 - [Client-Side DoS via Unbounded Input]
**Vulnerability:** The application accepted user text input via `window.prompt` (for `teleprompterCustomText`) and `<textarea>` elements (for `roadCustomText` and `prismCustomText`) without any length limits. Pasting extremely large strings (e.g., millions of characters) could cause severe UI lag, memory exhaustion, or a browser crash, resulting in a Denial of Service.
**Learning:** Even client-side applications can suffer from Denial of Service (DoS) attacks if they try to render, split, or process unbounded user input, particularly in tight render loops or heavy text processing tasks like syllabification.
**Prevention:** Always enforce reasonable bounds on user-supplied text. Apply HTML attributes like `maxlength="5000"` to text areas and validate/truncate prompt input using `.substring(0, 5000)` before processing it.
