...
## 2024-05-24 - [DOM-based XSS Risk via href property assignments]
**Vulnerability:** Tainted URL strings (like `window.location.href` or dynamically built URLs) were being directly assigned to element `.href` properties (e.g., `link.href = window.location.href`). This allowed DOM-based XSS attacks via dangerous protocol injection (e.g., `javascript:`, `data:`, `vbscript:`).
**Learning:** Even when avoiding `innerHTML` and using safe DOM APIs like `.href`, assigning unvalidated external or dynamic input to URL properties is dangerous. Attackers can craft URLs using the `javascript:` protocol that execute arbitrary code when clicked.
**Prevention:** All assignments to `.href` with potentially tainted URLs must be wrapped with a robust sanitization function. Created `sanitizeUrl` in `dsp-utils.js` that checks for and blocks dangerous protocols using a strict regex (`/^(%20|\s)*(javascript|data|vbscript):/i`), allowing safe protocols like `http:`, `https:`, and `blob:`.
