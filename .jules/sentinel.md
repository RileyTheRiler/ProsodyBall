...
## 2024-05-29 - [DOM-based XSS via direct .href assignment]
**Vulnerability:** Found multiple instances in `app.js` where untrusted URLs like `window.location.href` or dynamically generated object URLs were assigned directly to a DOM element's `.href` property (e.g. `link.href = window.location.href`). This allowed DOM-based XSS via injection of dangerous protocols like `javascript:`.
**Learning:** Direct assignment of tainted URL sources to `.href` properties on safe DOM elements (like `<a>`) is still vulnerable to DOM-based XSS if the URL starts with a dangerous protocol (e.g., `javascript:`, `data:`). It's not enough to avoid `innerHTML`; the URLs themselves must be validated.
**Prevention:** Always wrap untrusted URL assignments to `.href` with the `sanitizeUrl` utility from `dsp-utils.js` to strip dangerous protocols.
