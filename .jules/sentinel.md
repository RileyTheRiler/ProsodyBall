...
## 2024-05-29 - [DOM-based XSS Risk via window.location]
**Vulnerability:** Found multiple instances where `window.location.href` and dynamically constructed URLs were directly assigned to `<a>` and `<button>` `.href` attributes without sanitization. While browsers URL-encode many characters, this pattern allows for dangerous URL schemes (like `javascript:`) if the base URL or parameters can be manipulated or if the app is embedded in a context where the parent controls the URL.
**Learning:** Relying on `URL` object parsing alone is insufficient for security if the protocol isn't verified. `window.location.href` is a tainted source and must be explicitly sanitized against an allowlist of safe protocols (`http:`, `https:`, `mailto:`, `tel:`, `blob:`, `file:`) before being bound to an anchor tag's `href` attribute to prevent DOM-based XSS.
**Prevention:** Always wrap dynamically constructed or untrusted URLs with a strict sanitization utility (like `sanitizeUrl`) that validates the protocol against an explicit allowlist before rendering them as links.
