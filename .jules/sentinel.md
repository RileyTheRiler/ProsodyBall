...
## 2024-06-12 - [DOM-based XSS Risk via unsanitized URLs]
**Vulnerability:** Direct assignments of tainted URLs (like `window.location.href` or dynamically generated blob URLs) to `.href` properties (e.g., `link.href = ...`) in `app.js` could allow DOM-based XSS via dangerous protocol injection (`javascript:`, `data:`, `vbscript:`).
**Learning:** Even when avoiding `innerHTML`, directly setting the `.href` attribute of an anchor element with untrusted input is a known XSS vector. The browser will execute javascript protocols when the link is clicked.
**Prevention:** Wrap all dynamic URL assignments to `.href` properties with the `sanitizeUrl` utility function from `dsp-utils.js` to ensure dangerous protocols are neutralized, returning `about:blank` for invalid inputs.
