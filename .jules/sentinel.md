...
## 2024-06-03 - [DOM-based XSS Risk via Dangerous Protocol Injection in .href assignments]
**Vulnerability:** Direct assignment of dynamic or potentially tainted URLs (e.g., `window.location.href`, `url.href`, `URL.createObjectURL()`) to `.href` properties on dynamically created link elements (`a.href`) creates a vector for DOM-based XSS via dangerous protocols like `javascript:`, `data:`, or `vbscript:`.
**Learning:** Even when avoiding `innerHTML`, directly setting the `.href` property does not prevent browsers from executing JavaScript if the URL begins with a dangerous protocol and a user clicks it. The browser natively encodes the URL but doesn't validate its scheme.
**Prevention:** Always wrap any dynamic or untrusted URL assignment to an `.href` property with a strict sanitization function (like `sanitizeUrl` from `dsp-utils.js`) that explicitly tests for and blocks dangerous protocols, returning `about:blank` or dropping the assignment if detected.
