...
## 2024-05-30 - [DOM-based XSS Risk via Dangerous Protocol Injection]
**Vulnerability:** Found `window.location.href` and dynamically generated object URLs being directly assigned to `HTMLAnchorElement.href` properties (`link.href = ...`) in `app.js` without any sanitization. If `window.location.href` contained a dangerous protocol (e.g., `javascript:`, `data:`, or `vbscript:`), an attacker could execute arbitrary scripts when the user clicks the link.
**Learning:** Even safe DOM assignment methods (like direct property assignment `link.href =`) are susceptible to DOM-based XSS if the data source contains a dangerous protocol scheme that the browser interprets and executes upon interaction.
**Prevention:** Any untrusted URL (especially those derived from `window.location` or user input) must be strictly validated before assignment to `href` or `src` attributes. Use a sanitization helper (`sanitizeUrl` in `dsp-utils.js`) that blocks URLs starting with dangerous schemes using a regex (`/^(%20|\s)*(javascript|data|vbscript):/i`) to enforce safe link behaviors.
