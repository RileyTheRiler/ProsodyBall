...
## 2026-04-18 - [DOM-based XSS Risk via href protocol injection]
**Vulnerability:** Found assignments to `.href` properties on dynamically created `<a>` elements using tainted URLs (`window.location.href`) without validation. This opens a potential DOM-based XSS vector if the URL contains dangerous protocols like `javascript:`, `data:`, or `vbscript:`.
**Learning:** Even when avoiding `innerHTML` and using safe DOM creation APIs like `document.createElement('a')`, direct assignment of untrusted or loosely-controlled strings to execution sinks like `.href` remains an XSS risk. Browsers will execute `javascript:` payloads when a user clicks these links.
**Prevention:** All assignments of dynamically generated or external URLs to `.href` properties must be wrapped with a validation helper like `sanitizeUrl` to strip dangerous protocols before assignment.
