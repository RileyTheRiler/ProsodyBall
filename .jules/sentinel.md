...
## 2024-05-30 - [DOM-based XSS Risk via href Assignment]
**Vulnerability:** Found direct assignment of potentially tainted URLs (like `window.location.href` and user-controlled blobs) to `.href` properties of dynamically created anchor (`<a>`) elements.
**Learning:** Even when avoiding `innerHTML` and safely constructing elements with `document.createElement`, assigning an untrusted string directly to an `.href` property can lead to DOM-based XSS via malicious protocol injection (e.g., `javascript:alert(1)`).
**Prevention:** Always validate and sanitize URLs before assigning them to `.href` properties. Use a utility function like `sanitizeUrl` to strip or replace dangerous protocols (`javascript:`, `data:`, `vbscript:`) with safe fallbacks like `about:blank`.
