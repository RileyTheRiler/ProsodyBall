...
## 2024-05-30 - [DOM-based XSS via dynamically constructed URLs]
**Vulnerability:** Found multiple DOM-based XSS vectors where dynamically constructed URLs, including `window.location.href`, were directly assigned to `.href` properties of anchor (`<a>`) elements.
**Learning:** `window.location.href` and other dynamically built URLs (even `URL.createObjectURL`) are untrusted sources. Assigning them directly to `.href` allows execution of dangerous protocols like `javascript:` if the URL can be manipulated.
**Prevention:** Use an explicit allowlist of safe protocols (e.g., `http:`, `https:`, `blob:`) via a custom `sanitizeUrl` utility and wrap all dynamic `.href` assignments. Fall back to `about:blank` for disallowed protocols.
