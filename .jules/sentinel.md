...
## 2024-05-24 - [DOM-based XSS via direct href assignment]
**Vulnerability:** Found unsanitized assignments to `.href` with `window.location.href` and blob URLs. If the URL contained malicious protocols (like `javascript:` or `data:`), this could lead to DOM-based XSS (Protocol Injection).
**Learning:** Assigning directly to `.href` does not sanitize the protocol. Even if you don't use `innerHTML`, assigning user-controlled or derived URLs (like `window.location.href`) directly to `.href` is an XSS vector.
**Prevention:** Always wrap direct assignments to `.href` with a sanitization function (`sanitizeUrl`) that neutralizes dangerous protocols (`javascript:`, `data:`, `vbscript:`), returning a safe fallback like `about:blank`.
