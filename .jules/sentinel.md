...
## 2024-05-15 - DOM-based XSS via un-sanitized .href assignments
**Vulnerability:** Direct assignment of potentially tainted URLs (like `window.location.href` or dynamically generated `blob:` URLs from user data) to `.href` properties on `<a>` elements creates a DOM-based XSS vulnerability if an attacker can control the protocol (e.g. injecting `javascript:`).
**Learning:** In client-side apps, reflecting the current URL (often done for "open in new tab" error handling) or generating blobs without validation skips browser built-in navigation protections.
**Prevention:** Wrap all direct `.href` assignments with a sanitizer function (like `sanitizeUrl` in `dsp-utils.js`) that explicitly blocks dangerous protocols (using a regex anchored at the beginning of the string: `/^(?:%20|\s)*(?:javascript|data|vbscript):/i`) while safely allowing `blob:` and standard HTTP protocols. Never use this for `.src` on media elements as it breaks playback.
