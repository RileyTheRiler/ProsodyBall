...
## 2024-04-17 - [DOM-based XSS Risk via href Assignment]
**Vulnerability:** Untrusted external URL sources (`window.location.href`) and dynamically created proxy URLs (`directUrl`) were assigned directly to the `href` attribute of `<a>` elements in `app.js` without any sanitization.
**Learning:** `window.location.href` and URLs constructed from it are tainted sources and cannot be inherently trusted. They can be manipulated to carry a malicious payload (like a `javascript:` protocol URI), leading to DOM-based XSS upon user interaction.
**Prevention:** Created a centralized `sanitizeUrl` utility in `dsp-utils.js` that explicitly filters out dangerous URI protocols like `javascript:`, `data:`, and `vbscript:`. Applied this utility whenever assigning an untrusted URL to an `href` property.
