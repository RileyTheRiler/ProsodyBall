...
## 2025-02-14 - [DOM-based XSS Risk via `.href` properties]
**Vulnerability:** Untrusted inputs (such as `window.location.href` or dynamic `URL.createObjectURL` values) were assigned directly to `.href` properties of dynamically created DOM elements (`<a>`). Although better than `innerHTML`, this is still vulnerable to DOM-based XSS if the input string includes a malicious protocol like `javascript:`.
**Learning:** Even direct property assignment can be vulnerable if the assigned value is a URL and the protocol is unchecked. Tainted URLs can inject arbitrary JavaScript execution vectors.
**Prevention:** Always sanitize URLs before assigning them to href or similar attributes, explicitly denying dangerous protocols (`javascript:`, `vbscript:`, `data:`). Use a centralized URL sanitization utility function.
