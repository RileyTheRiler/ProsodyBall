...
## 2026-03-24 - [DOM-based XSS Risk via unsafe URL assignments]
**Vulnerability:** Direct assignments of tainted URLs (like `window.location.href` or dynamically generated URLs) to `.href` and `.src` properties in `app.js` present an XSS vector via dangerous protocols (`javascript:`, `data:`).
**Learning:** Directly assigning user-controlled or dynamically generated strings to `.href` or `.src` bypasses some DOM protections if the string contains a dangerous protocol. It is necessary to strictly evaluate the beginning of URLs.
**Prevention:** Wrap direct URL assignments to `.href` and `.src` with a `sanitizeUrl` utility function that identifies dangerous protocols and safely handles valid application states like `blob:` URLs.
