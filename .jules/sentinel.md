...
## 2024-05-24 - DOM-based XSS via location.href
**Vulnerability:** Untrusted inputs like `window.location.href` were being directly assigned to `href` attributes, allowing potential DOM-based XSS if the current URL contained a `javascript:` payload.
**Learning:** `blob:` URLs generated by `URL.createObjectURL()` are inherently safe from protocol-based XSS. Sanitizing them is unnecessary security theater.
**Prevention:** Use a lightweight regex `sanitizeUrl` function to block dangerous protocols like `javascript:`, `data:`, and `vbscript:` on untrusted URLs before assigning them to `href`, but exclude generated `blob:` URLs.
