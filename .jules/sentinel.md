...
## 2024-05-29 - [DOM-based XSS Risk via Missing URL Sanitization]
**Vulnerability:** Found multiple instances in `app.js` where untrusted or dynamically constructed URLs (e.g., `window.location.href`) were directly assigned to `.href` properties of anchor (`<a>`) elements. This poses a DOM-based XSS risk if the current URL contains malicious payloads, such as a `javascript:` scheme.
**Learning:** URL parameters or variables derived from `window.location` (like `href`) must never be trusted. Even if constructed client-side and assigned to DOM properties instead of `innerHTML`, they can still execute malicious code if they bypass scheme restrictions.
**Prevention:** Always wrap untrusted or dynamically constructed URLs with a URL sanitization utility (like the newly added `sanitizeUrl()` in `dsp-utils.js`) before assigning them to `.href` properties. The sanitizer should use the native `URL` constructor to validate the protocol scheme against an allowlist (e.g., `http:`, `https:`, `blob:`) and default to `about:blank` for invalid or unsafe URIs.
