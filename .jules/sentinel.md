...
## 2024-05-28 - [DOM-based XSS via window.location.href eliminated]
**Vulnerability:** Untrusted or dynamically constructed URLs derived from `window.location.href` were directly assigned to `.href` properties of links without validation, allowing potential DOM-based XSS if the current URL parameters or hash contained malicious payloads like `javascript:`.
**Learning:** `window.location.href` acts as an untrusted source that can be manipulated by attackers. Directly assigning it to an executable sink like `a.href` is a security risk.
**Prevention:** All dynamically constructed or untrusted URLs should be passed through a strict protocol allowlist validator (`sanitizeUrl`) before being rendered to DOM properties or attributes.
