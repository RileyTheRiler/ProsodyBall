...
## 2024-05-30 - [DOM-based XSS Risk via Direct URL Assignment to .href]
**Vulnerability:** Found `link.href = ...` being assigned directly with unvalidated URLs like `window.location.href` and dynamically generated blob URLs. This presents a potential DOM-based XSS risk if the URL protocol can be manipulated to `javascript:`, `data:`, or `vbscript:`.
**Learning:** Directly assigning tainted URLs (like `window.location.href` or user-controlled blob URLs) to `.href` properties is unsafe, even when using safe DOM construction (`document.createElement('a')`). Browsers will execute dangerous protocols (`javascript:alert(1)`) if the user clicks the link.
**Prevention:** Direct assignments of tainted URLs to `.href` properties must be wrapped with a robust `sanitizeUrl` utility function (e.g., from `dsp-utils.js`) to strictly validate and block dangerous protocols (`javascript:`, `data:`, `vbscript:`), falling back to `about:blank` for matched strings.
