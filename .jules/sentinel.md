...
## 2026-05-27 - [DOM-based XSS Risk via Dangerous Protocol Injection]
**Vulnerability:** Tainted inputs like `window.location.href` and dynamically constructed blob URLs were directly assigned to `HTMLAnchorElement.href` properties (e.g., `link.href = window.location.href`) in `app.js`. If an attacker manipulates the URL or the source data to start with a dangerous protocol (e.g., `javascript:`, `data:`), they can execute arbitrary scripts when the user clicks the link.
**Learning:** Directly assigning URL sources to `.href` without validation is a major DOM-based XSS vulnerability. Modern browsers do not automatically block dangerous protocols on `.href` assignments. Relying on the context (like an iframe error handler) doesn't guarantee the URL source is safe.
**Prevention:** All assignments to `.href` using external or dynamic data must be wrapped in a sanitization function that strictly rejects or strips dangerous protocols. The `sanitizeUrl` function in `dsp-utils.js` was used to enforce this by replacing matching tainted URLs with `about:blank`.
