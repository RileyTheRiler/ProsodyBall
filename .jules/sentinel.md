...
## 2024-05-24 - [DOM-based XSS via window.location.href in property assignments]
**Vulnerability:** Found `window.location.href` being directly assigned to `link.href` properties without sanitization in `app.js`. If the URL contained maliciously crafted `javascript:` or `data:` payloads, this could lead to XSS execution when the link is clicked.
**Learning:** Even though `link.href = ...` is safer than `innerHTML` against tag injection, it does NOT protect against dangerous protocol execution (e.g., `javascript:alert(1)`). All untrusted URLs must have their protocol validated.
**Prevention:** Always wrap untrusted URL strings with a protocol sanitization utility (e.g., `sanitizeUrl()`) before assigning them to `href` or `src` attributes. The sanitizer should check for and block protocols like `javascript:`, `data:`, and `vbscript:`.
