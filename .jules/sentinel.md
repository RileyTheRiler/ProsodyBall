...
## 2025-05-28 - [DOM-based XSS via dangerous protocol injection]
**Vulnerability:** Found multiple direct assignments to `.href` properties (such as `link.href`, `a.href`) using potentially tainted sources (like `window.location.href`, `url`) without URL sanitization.
**Learning:** Directly assigning tainted URLs (especially those derived from the current page's URL or user inputs) to `.href` properties allows DOM-based XSS if the URL contains dangerous protocols (`javascript:`, `data:`, `vbscript:`). Even when building elements programmatically using safe DOM APIs instead of `innerHTML`, `.href` values still require validation.
**Prevention:** Use a dedicated URL sanitizer function (e.g., `sanitizeUrl` using a regex check `^(%20|\s)*(javascript|data|vbscript):`) to validate and filter the `href` attribute value of link elements. Do not apply this to `.src` for `blob:` URLs used by media elements as it would block intended functionality.
