...
## 2026-05-05 - [Client-side DoS Risk via Unconstrained Inputs]
**Vulnerability:** Unconstrained user inputs (`textarea` and `window.prompt`) could accept massive strings. When these strings are processed in per-frame rendering loops (like `renderTeleprompter`), it can cause CPU saturation and freeze the UI (client-side DoS).
**Learning:** Even if input doesn't pose an XSS threat, its length can become a performance/availability vulnerability when it feeds into hot loops that parse or render the string 60 times a second.
**Prevention:** Always apply length constraints (e.g., `maxlength="5000"` for textareas, and `.substring(0, 5000)` for scripted prompts) to untrusted text inputs, especially those used in real-time visualizations.
