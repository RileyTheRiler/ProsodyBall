...
## 2024-07-08 - Added URL Sanitization Helper
**Vulnerability:** Unsafe assignments to `.href` properties can result in DOM-based XSS when user-supplied inputs trigger `javascript:` or `data:` URIs.
**Learning:** `app.js` routinely assigns `window.location.href`, `url.href`, or blob URLs directly to dynamically created `<a>` tags. While some of this data originates securely, defense-in-depth requires sanitizing all potentially dynamic URL assignments.
**Prevention:** Always use a central `sanitizeUrl` utility to wrap `.href` assignments, stripping out dangerous protocols like `javascript:`, `vbscript:`, and `data:` using regex: `/^(%20|\s)*(javascript|data|vbscript):/i`. Be careful to allow `blob:` URIs, which are heavily used in this application for audio playback and download.
