...
## 2024-05-30 - [DOM-based XSS via direct element href assignment]
**Vulnerability:** Found DOM-based XSS vulnerabilities in `app.js` where untrusted sources (e.g., `window.location.href`) were directly assigned to `.href` properties of programmatically created anchor elements (e.g., `link.href = window.location.href`).
**Learning:** While using `.createElement` and `.href` properties instead of `innerHTML` is a great first step, it does not prevent DOM-based XSS when assigning an untrusted string that begins with dangerous protocols like `javascript:`, `data:`, or `vbscript:`. If `window.location.href` contains a `javascript:` payload, clicking the generated link will still execute it.
**Prevention:** Always pass untrusted URL strings through a robust sanitization function (like `sanitizeUrl`) that strips or blocks dangerous protocols before assigning them to sensitive DOM sink properties such as `.href`, `.src`, or `.formAction`.
