...
## 2026-04-23 - [DoS Risk via Unbounded Text Input]
**Vulnerability:** User text inputs for teleprompter and custom passages (via `window.prompt` and `<textarea>` elements) lacked length restrictions. Pasting excessively large strings could lead to performance degradation, high memory consumption, or browser tab crashes (client-side DoS) when the application attempted to render or process the text.
**Learning:** Any user input that directly feeds into intensive client-side rendering loops (like the teleprompter overlay or prism analyzer) must have upper bounds, even if the application is purely client-side without a backend. Relying on users to input reasonable amounts of text is insufficient.
**Prevention:** Always enforce reasonable length limits on text inputs (e.g., using `.substring()` for JS prompts or the `maxlength` attribute for HTML form elements) to ensure the application remains performant and robust against large inputs.
