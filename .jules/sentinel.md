...
## 2026-06-17 - [DOM-based XSS Risk via link.href]
**Vulnerability:** Tainted URLs like `window.location.href` or dynamic blob URLs were assigned directly to link `.href` properties without sanitization, leading to DOM-based XSS via dangerous protocol injection (`javascript:`, `data:`, `vbscript:`).
**Learning:** Even safe DOM properties like `link.href` can execute JavaScript if prepended with dangerous protocols. Tainted inputs must always be validated and sanitized before assignment.
**Prevention:** Wrap all tainted URL assignments to `.href` properties with the `sanitizeUrl` utility function from `dsp-utils.js` to strip dangerous protocols.
