...
## 2026-03-24 - [DOM-based XSS via direct direct URL assignment]
**Vulnerability:** Found DOM-based XSS vulnerabilities in `app.js` where untrusted URLs like `window.location.href` or its parameters were being directly assigned to `.href` properties on dynamically created links (`<a>`).
**Learning:** Assigning an untrusted URL directly to the `.href` attribute is dangerous even if `innerHTML` is not used. A malicious URL (e.g. `javascript:alert(1)`) assigned to `.href` will execute arbitrary JavaScript when the link is clicked.
**Prevention:** Always sanitize dynamically constructed URLs using a strict protocol allowlist (e.g. `http:`, `https:`, `mailto:`, `tel:`, `file:`, `blob:`) before assigning them to DOM `.href` properties to prevent malicious schemes.
