...
## 2024-05-30 - [DOM-based XSS Risk via Unsafe href Assignments]
**Vulnerability:** Found multiple instances in `app.js` where dynamic sources like `window.location.href` were assigned directly to `link.href` properties. While modern browsers encode many characters, assigning to `href` does not sanitize dangerous protocols (e.g., `javascript:`), posing a DOM-based XSS risk if the URL contains a payload.
**Learning:** Assigning to `href` is safer than `innerHTML`, but it is not a complete defense against XSS. Dangerous protocols must be explicitly filtered out, especially when dealing with untrusted or partially trusted sources like `window.location.href`.
**Prevention:** Use a dedicated `sanitizeUrl` utility that explicitly blocks or maps dangerous protocols (`javascript:`, `data:`, `vbscript:`) to safe values like `about:blank` before assigning them to DOM element properties.
