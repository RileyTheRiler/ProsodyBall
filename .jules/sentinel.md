...
## 2024-05-24 - [DOM-based XSS Risk via .href assignments]
**Vulnerability:** Found multiple instances where unvalidated URLs (such as `window.location.href` and dynamically generated blob URLs) were assigned directly to the `.href` property of `<a>` tags. This creates a DOM-based XSS vulnerability if an attacker injects a malicious protocol (e.g., `javascript:alert(1)`) via manipulated URL parameters.
**Learning:** Directly assigning tainted URLs to `.href` properties allows for DOM-based XSS via dangerous protocol execution. Browser's default URL encoding does not mitigate this if the payload is in the protocol part. The `sanitizeUrl` function must block dangerous protocols (`javascript:`, `data:`, `vbscript:`) while explicitly allowing `blob:` URLs, as the app relies on `URL.createObjectURL()` for features like downloading voice recordings.
**Prevention:** All dynamic and untrusted URL assignments to `.href` properties (e.g., `link.href = ...`) must be wrapped with the `sanitizeUrl` utility function from `dsp-utils.js` to prevent execution of malicious protocols.
