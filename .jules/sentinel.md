...
## 2024-05-30 - [DOM-based XSS Risk via direct location.href assignment]
**Vulnerability:** Untrusted string sources like `window.location.href` were being assigned directly to DOM properties (like `link.href`) without sanitization, leading to a DOM-based XSS risk from dangerous protocols like `javascript:`.
**Learning:** Even internal properties or locations that appear safe can contain arbitrary user inputs in query parameters or hash segments. Direct assignment to properties like `href` creates a vulnerable execution context if clicked.
**Prevention:** Always use a `sanitizeUrl` helper when assigning untrusted data to protocol-sensitive properties to explicitly block `javascript:`, `data:`, and `vbscript:` protocols while allowing safe ones.
