...
## 2026-07-09 - [DOM-based XSS via direct .href assignments]
**Vulnerability:** Found multiple instances where tainted URLs (like `window.location.href` or dynamically generated blob URLs) were directly assigned to the `.href` property of anchor tags (`<a>`) in `app.js`. This allows DOM-based XSS if the URL contains a dangerous protocol like `javascript:`.
**Learning:** Even when avoiding `innerHTML`, direct assignment of untrusted strings to execution sinks like `.href` or `.src` is dangerous. Browsers will execute `javascript:` URIs when a user clicks a link constructed this way.
**Prevention:** Always wrap direct assignments to `.href` properties with a sanitization utility function that strictly blocks dangerous protocols (e.g., `javascript:`, `data:`, `vbscript:`) but allows safe ones like `http:` and `blob:`.
