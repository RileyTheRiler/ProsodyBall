...
## 2024-05-25 - [DOM-based XSS Risk via redundantly wrapping window.location.href in sanitizeUrl]
**Vulnerability:** We attempted to sanitize `window.location.href` assignments to `link.href` using a custom `sanitizeUrl` utility to prevent XSS.
**Learning:** Sanitizing `window.location.href` is security theater. The `window.location.href` variable inherently reflects the browser's current context protocol (usually HTTP/HTTPS). In a standard web context, it cannot independently contain an exploit like `javascript:` unless the page is already executing on that protocol. Additionally, using `sed` to patch the massive 1000+ character import block at the top of the file without care caused duplicate imports and syntax errors, breaking lint checks.
**Prevention:** Avoid blindly applying security patterns to safe/redundant sources. Also, never commit scratchpad files (like `temp.txt`) or unrelated lockfile changes (`pnpm-lock.yaml`) generated during ad-hoc testing to maintain a clean and reviewed patch state.
