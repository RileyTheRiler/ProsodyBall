...
## 2026-03-26 - [DOM-based XSS Risk via href protocol handling]
**Vulnerability:** Found `window.location.href` and unverified URLs (`directUrl`) being directly assigned to `link.href` in `app.js`. If a malicious `javascript:` URL was somehow parsed or manipulated into the URL bar, assigning it to `.href` executes the javascript payload upon click.
**Learning:** While assigning to `link.href` prevents typical DOM XSS via HTML characters (like quotes `<`), it does not prevent DOM-based XSS if the URL string begins with a dangerous protocol like `javascript:`. Protocol sanitization via whitelist or regex is strictly required even when using safe DOM assignments.
**Prevention:** Always wrap untrusted or location-derived URLs in a helper function like `sanitizeUrl` to filter out dangerous protocols before assignment to `.href` or `.src`.
