...
## 2026-05-22 - [Accessible Info Tooltips]
**Learning:** Found that info tooltips were not correctly associated with their trigger buttons for screen reader users, lacking proper ARIA properties.
**Action:** Ensure dynamic tooltips assign unique IDs to the popup element, map them to the trigger via `aria-describedby`, and manage the `aria-expanded` attribute on the trigger based on visibility state to improve screen reader context.
