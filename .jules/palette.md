...
## 2026-05-19 - [Inaccessible Tooltips]\n**Learning:** Found that info tooltips triggered by buttons (e.g., `.info-trigger` and `.info-popup`) lacked unique IDs and ARIA associations, making them invisible to screen readers.\n**Action:** When implementing interactive tooltips, dynamically assign a unique ID to the popup container and set `aria-describedby` on the trigger to associate them. Additionally, manage the `aria-expanded` attribute on the trigger (true when visible, false when hidden) to communicate state to screen readers.
