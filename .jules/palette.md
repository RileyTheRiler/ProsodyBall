...
## 2024-06-19 - [Inaccessible custom toggle switches due to duplicate IDs]
...
## 2024-06-19 - [Inaccessible info tooltips]
**Learning:** Found that custom info tooltips (`.info-trigger` and `.info-popup`) lacked proper ARIA associations, meaning screen readers couldn't identify the popup content associated with the trigger. Furthermore, `aria-expanded` was not dynamically managed.
**Action:** Ensure custom tooltips have dynamically generated unique IDs on the popup, and the trigger explicitly sets `aria-describedby` pointing to that ID. Additionally, the trigger must toggle `aria-expanded` based on the tooltip's visibility state.
