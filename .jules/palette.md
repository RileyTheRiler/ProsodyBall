...
## 2026-05-25 - [Inaccessible Info Tooltips]
**Learning:** Found that info tooltips triggered by buttons (.info-trigger) lacked proper programmatic association with their content and did not communicate their expanded state, leaving screen reader users without context about the tooltip content or when it was visible.
**Action:** When creating info tooltips, always assign a unique ID to the popup, link the trigger using `aria-describedby`, and dynamically update the trigger's `aria-expanded` attribute based on the tooltip's visibility state.
