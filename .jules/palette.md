...
**Learning:** Discovered that the accessible info tooltips using `.info-trigger` and `.info-popup` lacked proper ARIA associations in their dynamically attached handlers. While they looked fine on hover/focus, screen readers weren't alerted to their expanded state or their described relationship.
**Action:** Always ensure that dynamically toggled informational popups have a unique ID, the trigger references it via `aria-describedby`, and the trigger dynamically updates its `aria-expanded` attribute based on visibility state.
