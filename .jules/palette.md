...
## $(date +%Y-%m-%d) - [Inaccessible Tab Widget]
**Learning:** Found that the "Help" tabs (`.help-tabs`) were built using standard `<button>` and `<div>` elements with visual `active` classes, but lacked semantic ARIA roles (`tablist`, `tab`, `tabpanel`) and relationships (`aria-controls`, `aria-labelledby`, `aria-selected`, `hidden`). This meant screen reader users could not perceive the relationship between the tabs or their associated content panels, rendering the tabbed interface invisible to assistive technologies.
**Action:** When creating or modifying tab widgets, strictly implement the `role="tablist"` pattern with `role="tab"` for buttons and `role="tabpanel"` for content. Dynamically manage the `aria-selected` attribute on tabs and the `hidden` attribute on inactive panels in JavaScript to accurately reflect the active state to screen readers.
