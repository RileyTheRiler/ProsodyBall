...
## $(date +%Y-%m-%d) - [Missing accessibility links between info tooltips and their triggers]
**Learning:** Found that info tooltips (`.info-popup`) had no programmatic association with their trigger buttons (`.info-trigger`), and their expansion state was not communicated to screen readers.
**Action:** When implementing custom tooltip components, dynamically assign unique IDs to the tooltip containers, set `aria-describedby` on the trigger buttons to point to those IDs, and ensure `aria-expanded` is toggled on the trigger when the tooltip is shown/hidden.
