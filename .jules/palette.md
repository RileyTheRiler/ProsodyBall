...
## $(date +%Y-%m-%d) - [Destructive Action Confirmation]
**Learning:** Found that deleting audio recordings in the Recordings drawer lacked any confirmation prompt, immediately destroying user data upon clicking the 'delete' icon.
**Action:** Always enforce explicit confirmation dialogs (e.g., using `window.confirm()`) before executing destructive actions on user-generated data to prevent accidental loss.
