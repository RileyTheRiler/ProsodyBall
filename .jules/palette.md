...
## 2026-05-02 - [Missing aria-pressed on toggle buttons]
**Learning:** Found that UI toggle buttons relying on the `.active` class for visual state (#cbToggle, #pauseCanvasBtn, #motionToggle) were missing the `aria-pressed` attribute, preventing screen readers from communicating their active state.
**Action:** When implementing or modifying toggle buttons that use CSS classes for state, always dynamically sync the `aria-pressed` attribute in JavaScript to ensure accurate state communication to assistive technologies.
