...
## 2026-05-04 - [Inaccessible Info Tooltips]
**Learning:** Discovered that info tooltips triggered by buttons (`.info-trigger`) lacked ARIA association (`aria-describedby`) with their popup containers and did not communicate their visibility state (`aria-expanded`) to screen readers.
**Action:** When implementing custom info tooltips, dynamically assign unique IDs to popups, link them using `aria-describedby` on the trigger, and manage `aria-expanded` true/false based on visibility.
