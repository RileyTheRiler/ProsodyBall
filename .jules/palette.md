...
## 2026-05-31 - [Inaccessible custom toggle switches due to duplicate IDs]
...
## 2026-05-31 - [Inaccessible info tooltips due to missing aria attributes]
**Learning:** Found that `.info-trigger` elements used for `.info-popup` tooltips lacked proper ARIA attributes (`aria-describedby` and `aria-expanded`), making them inaccessible and confusing for screen reader users as they had no programmatic connection to the text they displayed when hovered or focused.
**Action:** When implementing custom info tooltips, always ensure the trigger element has an explicit `aria-describedby` linking it to the tooltip's unique ID, and a dynamically updated `aria-expanded` attribute to announce visibility state changes.
