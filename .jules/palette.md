...
## 2024-05-31 - Tabbed Widget Accessibility Pattern
**Learning:** For custom tab implementations like `.help-tabs`, screen reader users require explicit ARIA roles (`tablist`, `tab`, `tabpanel`) to understand the component structure, and relationship attributes (`aria-controls`, `aria-labelledby`) to navigate between tabs and their corresponding panels.
**Action:** Implemented the ARIA tabs pattern by adding static roles to HTML, mapping `aria-controls` on the tab to the panel's `id`, mapping `aria-labelledby` on the panel to the tab's `id`, and updating `aria-selected` dynamically in the JavaScript click handler.
