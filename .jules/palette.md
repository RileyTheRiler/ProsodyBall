...
## 2026-05-05 - [Missing ARIA attributes on Info Tooltips]
**Learning:** Found that info tooltips triggered by buttons in the app lacked dynamic `aria-describedby` linkage and `aria-expanded` state management, leaving screen reader users without essential context.
**Action:** When implementing info tooltips triggered by buttons, dynamically assign a unique ID to the popup container and set `aria-describedby` on the trigger to associate them. Manage the `aria-expanded` attribute on the trigger to communicate state to screen readers.
