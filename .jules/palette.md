...
## 2026-03-17 - [Missing aria-pressed on toggle buttons]
**Learning:** Found that toggle buttons using the `.active` CSS class to show visual state changes (like `#cbToggle`, `#pauseCanvasBtn`, and `#motionToggle`) lacked a dynamically updated `aria-pressed` attribute, preventing screen readers from conveying their active state.
**Action:** When using a CSS class like `.active` to indicate the pressed/toggled state of a button, always initialize `aria-pressed="false"` (or `"true"`) in HTML and dynamically update it in JavaScript synchronously with the class toggle.
