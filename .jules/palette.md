...
## 2024-06-25 - [Inaccessible custom toggle switches due to duplicate IDs]
...
## 2024-06-25 - [Inaccessible Tooltip Associations]
**Learning:** Found that custom info tooltips (`.info-trigger` and `.info-popup`) were not programmatically associated, leaving screen reader users without context when focusing on the trigger buttons. Furthermore, the `aria-expanded` state was not dynamically updated when the tooltip was shown or hidden.
**Action:** When implementing custom tooltips, always ensure the popup element has a unique ID, the trigger references it via `aria-describedby`, and the trigger dynamically updates its `aria-expanded` attribute based on the visibility state of the popup.
