...
## $(date +%Y-%m-%d) - [Missing WAI-ARIA tab semantics on help panel]
**Learning:** Discovered that the help tooltip's tab interface (`.help-tabs`) lacked essential WAI-ARIA semantics (like `role="tablist"`, `role="tab"`, and `role="tabpanel"`), and did not dynamically update `aria-selected` or use the `hidden` attribute. This structure prevented screen reader users from understanding the relationship between the tabs and the content panels, and exposed hidden content inappropriately.
**Action:** Always implement full WAI-ARIA tab semantics when creating custom tabbed interfaces. This includes explicitly linking tabs to panels via `aria-controls` and `aria-labelledby`, dynamically updating `aria-selected`, and ensuring inactive panels use the `hidden` HTML attribute to remove them from the accessibility tree.
