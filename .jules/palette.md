...
## 2026-05-12 - [Syncing aria-pressed for Toggle Buttons]
**Learning:** Discovered that toggle buttons relying entirely on the `.active` CSS class for their visual state (`#cbToggle`, `#pauseCanvasBtn`, `#motionToggle`, `#vibToggle`) were not explicitly communicating their pressed state to screen readers.
**Action:** When implementing custom toggle buttons that use CSS classes for state management, ensure the `aria-pressed` attribute is both initialized correctly in HTML and dynamically synced via JavaScript alongside the CSS class updates.
