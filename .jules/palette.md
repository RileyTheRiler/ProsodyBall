...
## 2025-05-18 - [Missing ARIA Roles and State on Custom Tab Interfaces]
**Learning:** Found that custom tabbed interfaces (like the `help-tabs` widget) were built using generic `<div>` and `<button>` elements without explicit ARIA roles or state attributes (`role="tablist"`, `role="tab"`, `role="tabpanel"`, `aria-selected`, `aria-controls`, `aria-labelledby`). This leaves screen reader users completely unaware of the relationship between the tabs and panels or which tab is currently active.
**Action:** Always implement semantic ARIA roles (`role="tablist"`, `role="tab"`, `role="tabpanel"`) and relationship attributes (`aria-controls`, `aria-labelledby`) when creating or modifying custom tab widgets. Furthermore, ensure JavaScript dynamically updates the `aria-selected` attribute on the tab buttons and conditionally toggles the `hidden` attribute on inactive panels.
