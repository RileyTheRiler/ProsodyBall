...
class VoxBallGame {
  ...
  updatePitchPilot(dt) {
    ...
    if (!this.isRunning) {
      ...
      // ⚡ Bolt Optimization: backward loop and in-place .splice() avoids per-frame array allocation and GC pressure
      for (let i = pp.barriers.length - 1; i >= 0; i--) {
        if (pp.barriers[i].x + pp.barriers[i].width <= -120) pp.barriers.splice(i, 1);
      }
