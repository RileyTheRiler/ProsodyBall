...
class VoxBallGame {
  ...
  renderPrismOverlay(dt) {
    ...
    if (progressFill && pr.syllables.length > 0) {
      ...
      // Replaced chained .filter() with a manual loop to prevent intermediate array allocation in per-frame rendering
      let crystallizedCount = 0;
      for (let i = 0; i < pr.syllables.length; i++) {
        if (pr.syllables[i].state === 'crystallized') crystallizedCount++;
      }

      if (crystallizedCount >= 2) {
        const stops = [];
        const step = Math.max(1, Math.floor(crystallizedCount / 5));
        let currentCrystallized = 0;
        let lastSyllable = null;

        for (let i = 0; i < pr.syllables.length; i++) {
          const s = pr.syllables[i];
          if (s.state === 'crystallized') {
            if (currentCrystallized % step === 0) {
              const pos = Math.round((currentCrystallized / (crystallizedCount - 1)) * 100);
        ...
        if (lastSyllable) {
          const lastSat = Math.round(40 + lastSyllable.vowelScore * 35);
          const lastLight = Math.round(45 + lastSyllable.confidence * 20);
          stops.push(`hsl(${Math.round(lastSyllable.hue)}, ${lastSat}%, ${lastLight}%) 100%`);
        }
