...
class VoxBallGame {
  ...
  setupUI() {
    ...
    if (isInIframe && iframeNotice) {
      ...
      link.href = sanitizeUrl(directUrl); // Security: sanitize untrusted URL parameter
      link.target = '_blank';
    ...
    const startGame = async () => {
      ...
      if (!selectedAudioFile && (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia)) {
        ...
        if (isInIframe) {
          const link = document.createElement('a');
          link.href = sanitizeUrl(window.location.href); // Security: sanitize untrusted window.location.href
          link.target = '_blank';
          link.rel = 'noopener noreferrer';
        ...
        try {
          const { stream, cleanup } = await startPhoneMicSession((status, code) => {
            if (status === 'waiting') {
              ...
              if (phoneMicUrlEl) { phoneMicUrlEl.href = sanitizeUrl(url.href); phoneMicUrlEl.textContent = url.href; phoneMicUrlEl.style.display = ''; } // Security: sanitize URL
