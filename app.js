      ...
      if (!selectedAudioFile && this.micInputPreferences.deviceId === 'phone-mic') {
        ...
        try {
          const { stream, cleanup } = await startPhoneMicSession((status, code) => {
            if (status === 'waiting') {
              const url = new URL('phone.html', sanitizeUrl(window.location.href));
              ...
              if (phoneMicUrlEl) { phoneMicUrlEl.href = sanitizeUrl(url.href); phoneMicUrlEl.textContent = sanitizeUrl(url.href); phoneMicUrlEl.style.display = ''; }
