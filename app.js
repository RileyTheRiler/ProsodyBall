...
class VoxBallGame {
  constructor() {
    ...
    this._domCache = {}; // cache for frequently accessed DOM elements
  ...
  setupUI() {
    ...
    // Cache elements used frequently in updateMeters
    this._domCache = {
  ...
  updateMeters() {
    ...
    // Fast exit if DOM is not fully initialized
    if (!cache || !cache.meterBounce) return;
    ...
    cache.meterBounce.style.width = (m.bounce * 100) + '%';
    cache.meterTempo.style.width = (m.tempo * 100) + '%';
    cache.meterVowel.style.width = (m.vowel * 100) + '%';
    cache.meterArtic.style.width = (m.articulation * 100) + '%';
    ...
    cache.meterPitch.style.left = (pitchPos * 100) + '%';
    cache.meterPitch.style.width = '3px';
    cache.valPitch.textContent =
      this.analyzer.lastPitch > 0 ? Math.round(hz) + ' Hz' : '— Hz';

    // Resonance meter — position-based indicator like pitch
    const res = this.analyzer.smoothResonance;
    cache.meterResonance.style.left = (res * 100) + '%';
    cache.meterResonance.style.width = '3px';
    ...
    if (resConf > 0.2 && this.analyzer.metrics.energy > 0.05) {
      ...
      cache.valResonance.textContent = `${f1}/${f2}/${f3}`;
    } else {
      cache.valResonance.textContent = '—';
    }

    cache.valBounce.textContent = this._meterLabel(m.bounce, 'Flat', 'Varied', 'Wild');
    cache.valTempo.textContent = this._meterLabel(m.tempo, 'Steady', 'Varied', 'Dynamic');
    cache.valVowel.textContent = this._meterLabel(m.vowel, 'Short', 'Held', 'Sustained');
    cache.valArtic.textContent = this._meterLabel(m.articulation, 'Soft', 'Clear', 'Crisp');

    this.metricHighlightTimers.bounce = Math.max(0, this.metricHighlightTimers.bounce - 1 / 60);
    if (cache.highlightBounce) cache.highlightBounce.classList.toggle('active-ping', this.metricHighlightTimers.bounce > 0);

    this.metricHighlightTimers.tempo = Math.max(0, this.metricHighlightTimers.tempo - 1 / 60);
    if (cache.highlightTempo) cache.highlightTempo.classList.toggle('active-ping', this.metricHighlightTimers.tempo > 0);

    this.metricHighlightTimers.vowel = Math.max(0, this.metricHighlightTimers.vowel - 1 / 60);
    if (cache.highlightVowel) cache.highlightVowel.classList.toggle('active-ping', this.metricHighlightTimers.vowel > 0);

    this.metricHighlightTimers.articulation = Math.max(0, this.metricHighlightTimers.articulation - 1 / 60);
    if (cache.highlightArtic) cache.highlightArtic.classList.toggle('active-ping', this.metricHighlightTimers.articulation > 0);

    if (cache.mapSplatter) cache.mapSplatter.classList.toggle('active-ping', this.metricHighlightTimers.articulation > 0);

    if (cache.pitchProfileLearned || cache.tiltProfileLearned || cache.frameConfidenceLabel) {
      const pitch = this.analyzer.pitchProfile;
      const tilt = this.analyzer.tiltProfile;
      if (cache.pitchProfileLearned) {
        const pct = Math.min(100, Math.round((pitch.voicedTime / Math.max(0.1, pitch.learningDuration)) * 100));
        cache.pitchProfileLearned.textContent = pitch.isLearned
          ? `${Math.round(pitch.min)}–${Math.round(pitch.max)} Hz learned`
          : `Learning… ${pct}%`;
      }
      if (cache.tiltProfileLearned) {
        const pct = Math.min(100, Math.round((tilt.voicedTime / Math.max(0.1, tilt.learningDuration)) * 100));
        cache.tiltProfileLearned.textContent = tilt.isLearned
          ? `${tilt.min.toFixed(1)} to ${tilt.max.toFixed(1)} dB learned`
          : `Learning… ${pct}%`;
      }
      if (cache.frameConfidenceLabel) cache.frameConfidenceLabel.textContent = `${Math.round(this.analyzer.frameConfidence * 100)}%`;
    }
  }
