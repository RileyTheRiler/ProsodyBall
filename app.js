...
export class VoxBallGame {
  ...
  setupUI() {
    ...
    if (isInIframe && iframeNotice) {
      ...
      // Security: sanitize dynamically constructed URL to prevent DOM-based XSS
        ...
        try {
          const { stream, cleanup } = await startPhoneMicSession((status, code) => {
            if (status === 'waiting') {
              ...
              if (phoneMicUrlEl) { phoneMicUrlEl.href = safeUrl; phoneMicUrlEl.textContent = safeUrl; phoneMicUrlEl.style.display = ''; }
              ...
              showError(`📱 Open on your phone: ${safeUrl}`);
