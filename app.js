...
class VoxBallGame {
  ...
  setupUI() {
    ...
    if (isInIframe && iframeNotice) {
      ...
      // Security: sanitize untrusted URLs before assignment to prevent DOM-based XSS
    ...
    const startGame = async () => {
      ...
      if (!selectedAudioFile && (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia)) {
        ...
        if (isInIframe) {
          const link = document.createElement('a');
          // Security: sanitize window.location.href before assignment to prevent DOM-based XSS
        ...
        try {
          const { stream, cleanup } = await startPhoneMicSession((status, code) => {
            if (status === 'waiting') {
              ...
              // Security: sanitize dynamically constructed URLs before assignment to prevent DOM-based XSS
