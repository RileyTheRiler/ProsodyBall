...
class VoxBallGame {
  ...
  setupUI() {
    ...
    if (isInIframe && iframeNotice) {
      ...
      // Security: Sanitize the directUrl to prevent DOM-based XSS when assigning to href
    ...
    const startGame = async () => {
      ...
      if (!selectedAudioFile && (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia)) {
        ...
        if (isInIframe) {
          const link = document.createElement('a');
          // Security: Sanitize window.location.href to prevent DOM-based XSS
        ...
        try {
          const { stream, cleanup } = await startPhoneMicSession((status, code) => {
            if (status === 'waiting') {
              ...
              // Security: Sanitize url.href to prevent DOM-based XSS when assigning to href
