...
class VoxBallGame {
  ...
  renderPrismOverlay(dt) {
    ...
    if (progressFill && pr.syllables.length > 0) {
      ...
      // ⚡ Bolt: Replace .filter() with manual loops for dynamic UI gradient calculations in per-frame rendering to prevent array allocation overhead.
      let cCount = 0;
      const sylLen = pr.syllables.length;
      for (let i = 0; i < sylLen; i++) {
        if (pr.syllables[i].state === 'crystallized') cCount++;
      }

      if (cCount >= 2) {
        const stops = [];
        const step = Math.max(1, Math.floor(cCount / 5));
        let cIdx = 0;
        let lastSyl = null;
        for (let i = 0; i < sylLen; i++) {
          const s = pr.syllables[i];
          if (s.state === 'crystallized') {
            if (cIdx % step === 0) {
              const pos = Math.round((cIdx / (cCount - 1)) * 100);
        ...
        if (lastSyl) {
          ...
          stops.push(`hsl(${Math.round(lastSyl.hue)}, ${lastSat}%, ${lastLight}%) 100%`);
        }
