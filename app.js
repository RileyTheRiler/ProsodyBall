...
const PITCH_GUIDES = [100, 150, 200, 250, 300]; // Static guide frequencies for canvas
...
class VoxBallGame {
  ...
  drawVoiceCanvasScene(prosodyGlow) {
    ...
    ctx.textAlign = 'left';
    // ⚡ Bolt: Use pre-allocated static array and inline traditional loop to prevent GC thrashing per-frame
    for (let i = 0; i < PITCH_GUIDES.length; i++) {
      const hz = PITCH_GUIDES[i];
      const norm = Math.max(0, Math.min(1, (hz - 80) / 220)); // 220 is (300 - 80)
