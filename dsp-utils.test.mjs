...
test('sanitizeUrl handles malicious protocols', () => {
...
test('sanitizeUrl allows safe URLs', () => {
  ...
  assert.equal(sanitizeUrl('http://test.com/path'), 'http://test.com/path');
  assert.equal(sanitizeUrl('/local/path'), '/local/path');
  assert.equal(sanitizeUrl('mailto:test@example.com'), 'mailto:test@example.com');
  assert.equal(sanitizeUrl('blob:http://localhost/12345'), 'blob:http://localhost/12345');
});

test('sanitizeUrl handles non-string inputs gracefully', () => {
  assert.equal(sanitizeUrl(null), 'about:blank');
  assert.equal(sanitizeUrl(undefined), 'about:blank');
  assert.equal(sanitizeUrl(123), 'about:blank');
});
