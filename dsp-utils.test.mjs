...
test('sanitizeUrl handles safely allowed urls', () => {
  assert.equal(sanitizeUrl('https://example.com'), 'https://example.com');
  assert.equal(sanitizeUrl('/path/to/file'), '/path/to/file');
  assert.equal(sanitizeUrl('blob:https://example.com/123'), 'blob:https://example.com/123');
});
...
test('sanitizeUrl handles non-string inputs safely', () => {
