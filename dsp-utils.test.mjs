...
test('sanitizeUrl handles valid URLs', () => {
  assert.equal(sanitizeUrl('https://example.com'), 'https://example.com');
  assert.equal(sanitizeUrl('http://test.org'), 'http://test.org');
  assert.equal(sanitizeUrl('/local/path'), '/local/path');
  assert.equal(sanitizeUrl('mailto:test@example.com'), 'mailto:test@example.com');
});

test('sanitizeUrl sanitizes dangerous protocols', () => {
...
test('sanitizeUrl handles empty and nullish inputs', () => {
